...
class TestContentAnalyzer:
    ...
    def test_categorization(self, analyzer):
        ...
        # Research content
        research_content = "This research paper presents findings from our scientific study."
    ...
    def test_completeness_scoring(self, analyzer):
        ...
        complete_analysis = analyzer.analyze_content(complete_content, complete_metadata)
        ...
        incomplete_analysis = analyzer.analyze_content(incomplete_content, incomplete_metadata)
    ...
    def test_tag_extraction(self, analyzer, sample_metadata):
        ...
        tags = analysis['tags']
        assert len(tags) > 0
        assert any('machine learning' in tag.lower() or 'artificial intelligence' in tag.lower() for tag in tags)
    ...
    def test_sort_by_relevance(self, analyzer):
        ...
        articles = [
            {'quality_score': 0.8, 'freshness_score': 0.9, 'title': 'High Quality, Fresh'},
            {'quality_score': 0.9, 'freshness_score': 0.5, 'title': 'High Quality, Old'},
            {'quality_score': 0.5, 'freshness_score': 0.8, 'title': 'Medium Quality, Fresh'}
        ]
