...
class SerperSearchProvider:
    ...
    def _initialize_serper(self):
        ...
        try:
            ...
            if sys.version_info < (3, 10):
                logger.warning(
                    "Python 3.9 detected - crewai_tools may have compatibility issues")
            ...
            for module_name, class_name in import_paths:
                try:
                    ...
                    logger.info(
                        f"Serper API tool initialized successfully via {module_name}")
                    return
            ...
            logger.warning(
                "SerperDevTool not available. Install with: pip install crewai-tools")
            self.serper_tool = None
    ...
    def is_available(self) -> bool:
        ...
        return self.serper_tool is not None

    def search(self, query: SearchQuery) -> List[SearchResult]:
        """Perform search using Serper API."""
        if not self.is_available():
            raise RuntimeError("Serper API not available")

        try:
            # Use the SerperDevTool to perform search
            search_result = self.serper_tool.run(search_query=query.query)

            # Parse the result - SerperDevTool returns a dict with organic
            # results
            if isinstance(search_result, dict) and 'organic' in search_result:
                results = []
                organic_results = search_result['organic'][:query.max_results]
                ...
                return [SearchResult(
                    title="Serper Search Result",
                    url="",
                    snippet=search_result[:500] + "..." if len(search_result) > 500 else search_result,
                    source=self.name
                )]
            else:
                logger.warning(
                    f"Unexpected Serper result type: {
                        type(search_result)}")
                return []
...
class DuckDuckGoSearchProvider:
            ...
            for topic in data.get('RelatedTopics', [])[:query.max_results]:
                if isinstance(topic, dict) and topic.get('Text'):
                    results.append(
                        SearchResult(
                            title=topic.get(
                                'Text', '').split(' - ')[0] if ' - ' in topic.get(
                                'Text', '') else topic.get(
                                'Text', ''), url=topic.get(
                                'FirstURL', ''), snippet=topic.get(
                                'Text', ''), source=self.name, metadata={
                                'type': 'related_topic'}))
    ...
    def is_available(self) -> bool:
        ...
        return True  # DuckDuckGo is generally always available
...
class KnowledgeBaseSearchProvider:
        ...
        try:
            ...
            for result in results:
                formatted_results.append(
                    SearchResult(
                        title=result.get(
                            'title', 'Knowledge Base Result'), url=result.get(
                            'url', ''), snippet=result.get(
                            'content', '')[
                            :200] + '...' if len(
                            result.get(
                                'content', '')) > 200 else result.get(
                                    'content', ''), source=self.name, metadata={
                                        'score': result.get('score'), 'type': 'vector_search'}))
    ...
    def search(self, query: SearchQuery) -> List[SearchResult]:
        """Perform search using knowledge base."""
        return self._perform_search(query)
...
class UnifiedSearchProvider:
    ...
    def search(self, query: str, max_results: int = 5,
               search_type: str = "web") -> List[SearchResult]:
...
def get_unified_search_provider() -> UnifiedSearchProvider:
    ...
    if _unified_search_provider is None:
        try:
            ...
            logger.warning(
                f"Failed to initialize unified search provider: {e}")
...
def search_web_with_alternatives(
    ...
    results = provider.search_with_alternatives(
        primary_query, fallback_queries, max_results)
