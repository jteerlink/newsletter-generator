...
class AgenticRAGAgent:
    ...
    def process_query(self,
        ...
        session.synthesized_response = self._synthesize_response(
            query, session.retrieval_steps)
    ...
    def _determine_next_retrieval(self,
        ...
        for strategy in planned_strategies:
            if strategy not in completed_strategies:
                return {
                    "query": self._adapt_query_for_strategy(
                        query,
                        strategy,
                        completed_steps),
                    "strategy": strategy,
    ...
    def _evaluate_information_sufficiency(
            self, query: str, retrieval_steps: List[RetrievalStep]) -> Dict[str, Any]:
        ...
        avg_confidence = sum(
            step.confidence for step in retrieval_steps) / len(retrieval_steps)
        ...
        return {
            ...
            "reason": f"Coverage: {total_results} results, {
                len(unique_sources)} sources, " f"avg confidence: {
                avg_confidence:.2f}"}

    def _synthesize_response(
            self,
            query: str,
            retrieval_steps: List[RetrievalStep]) -> str:
        ...
        synthesis_prompt = self.reasoning_templates["synthesis_planning"].format(
            query=query,
            # Top 10 most confident results
            information=json.dumps(all_info[:10], indent=2)
        )
    ...
    def _calculate_session_confidence(
            self, session: AgenticRAGSession) -> float:
    ...
    def _extract_sources(
            self,
            retrieval_steps: List[RetrievalStep]) -> List[str]:
    ...
    def _evaluate_retrieval_results(
            self, results: List[Dict[str, Any]], query: str) -> Dict[str, Any]:
...
def create_agentic_rag_agent(
        vector_store: ChromaStorageProvider) -> AgenticRAGAgent:
