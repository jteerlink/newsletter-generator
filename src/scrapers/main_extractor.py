...
from src.scrapers.web_scraper import SmartWebScraper
...
def main():
    ...
    parser.add_argument(
        "--config", default="sources.yaml", help="Configuration file path"
    )
    parser.add_argument("--output", default="output", help="Output directory")
    parser.add_argument(
        "--selenium", action="store_true", help="Use Selenium for web scraping"
    )
    parser.add_argument(
        "--max-articles", type=int, default=50, help="Max articles per source"
    )

    # Extraction modes
    parser.add_argument(
        "--rss-only", action="store_true", help="Extract from RSS sources only"
    )
    parser.add_argument(
        "--web-only", action="store_true", help="Extract from websites only"
    )
    parser.add_argument("--category", help="Extract from specific category only")

    # Export options
    parser.add_argument(
        "--export", choices=["json", "csv", "excel"], help="Export articles"
    )
    parser.add_argument(
        "--export-recent", type=int, help="Export articles from last N hours"
    )

    # Maintenance
    parser.add_argument(
        "--cleanup", action="store_true", help="Remove duplicates and old articles"
    )
    parser.add_argument(
        "--report", action="store_true", help="Generate extraction report"
    )

    # Info
    parser.add_argument(
        "--list-sources", action="store_true", help="List all configured sources"
    )
    ...
    try:
        ...
        logprint("\n=== Extraction Summary ===")
        logprint(
            f"Total articles extracted: {results['extraction_stats']['total_articles']}"
        )
        logprint(
            f"New articles stored: {results['processing_results']['new_articles']}"
        )
        logprint(
            f"Duplicate articles: {results['processing_results']['duplicate_articles']}"
        )
        logprint(
            f"Successful sources: {results['extraction_stats']['successful_sources']}"
        )
        ...
        if results["extraction_stats"]["errors"]:
            ...
            for error in results["extraction_stats"]["errors"][
                :5
            ]:  # Show first 5 errors
