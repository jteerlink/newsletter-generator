...
class NewsExtractor:
    ...
    def extract_news(self, sources: List[str] = None, max_articles: int = 50) -> Dict[str, Any]:
        """
        ...
        if sources:
            ...
            config.sources = {name: source for name, source in config.sources.items() if name in sources}
    ...
    def _extract_rss(self, source_config: SourceConfig, max_articles: int) -> List[Dict[str, Any]]:
    ...
    def _extract_web(self, source_config: SourceConfig, max_articles: int) -> List[Dict[str, Any]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract news from configured sources")
    parser.add_argument(
        "--sources", 
        nargs="+", 
    ...
    logger.info(f"Extracted {results['total_articles']} articles from {results['sources_processed']} sources")
