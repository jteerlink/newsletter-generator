...
class ContentAnalyzer:
    ...
    def __init__(self):
        # Quality scoring weights (Phase 1.2 improved)
        ...
        # Source reliability scores (can be updated based on domain knowledge)
        ...
        self.category_keywords = {
            'AI/ML': ['artificial intelligence', 'machine learning', 'deep learning', 'neural network', 'AI', 'ML', 'algorithm'],
            'Research': ['research', 'study', 'paper', 'academic', 'scientific', 'experiment'],
            'Industry': ['company', 'startup', 'business', 'industry', 'market', 'product'],
            'Technology': ['technology', 'tech', 'software', 'hardware', 'system', 'platform'],
            'Conference': ['conference', 'workshop', 'symposium', 'presentation', 'talk'],
    ...
    def _assess_source_reliability(self, metadata: Dict[str, Any]) -> float:
        ...
        # Check source name
        for reliable_domain, score in self.source_reliability_scores.items():
            if reliable_domain.replace('.org', '').replace('.com', '') in source.lower():
                return score
    ...
    def _assess_completeness(self, content: str, metadata: Dict[str, Any]) -> float:
        ...
        # Check word count (more content = more complete)
    ...
    def _calculate_quality_score(self, analysis: Dict[str, Any]) -> float:
        ...
        # Source reliability
        score += analysis['source_reliability'] * self.quality_weights['source_reliability']
        # Freshness
        ...
        # Cap score for very short content
        if analysis['word_count'] < 20:
            score = min(score, 0.4)
        # Cap score for incomplete content
        if analysis['completeness_score'] < 0.5:
            score = min(score, 0.5)
        return round(score, 3)
    
    def _enrich_metadata(self, metadata: Dict[str, Any], analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def filter_by_quality(self, articles: List[Dict[str, Any]], min_quality: float = 0.5) -> List[Dict[str, Any]]:
        """Filter articles by minimum quality score."""
        return [article for article in articles if article.get('quality_score', 0) >= min_quality]
    
    def sort_by_relevance(self, articles: List[Dict[str, Any]], query: str = None) -> List[Dict[str, Any]]:
