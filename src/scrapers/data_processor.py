...
class DataProcessor:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                cursor.execute('ALTER TABLE articles ADD COLUMN quality_score REAL DEFAULT 0.0')
            except sqlite3.OperationalError:
                pass  # Column already exists
            
            try:
                cursor.execute('ALTER TABLE articles ADD COLUMN readability_score REAL DEFAULT 0.0')
            except sqlite3.OperationalError:
                pass
            
            try:
                cursor.execute('ALTER TABLE articles ADD COLUMN source_reliability REAL DEFAULT 0.5')
            except sqlite3.OperationalError:
                pass
            
            try:
                cursor.execute('ALTER TABLE articles ADD COLUMN freshness_score REAL DEFAULT 0.5')
            except sqlite3.OperationalError:
                pass
            
            try:
                cursor.execute('ALTER TABLE articles ADD COLUMN completeness_score REAL DEFAULT 0.0')
            except sqlite3.OperationalError:
                pass
    ...
    def process_articles(self, articles: List[Article]) -> Dict[str, Any]:
        ...
        for article in articles:
            try:
                ...
                is_new, quality_info = self.store_article_with_analysis(article, existing_hashes)
    ...
    def store_article_with_analysis(self, article: Article, existing_hashes: List[str]) -> tuple[bool, Dict[str, Any]]:
        ...
        content = getattr(article, 'raw_content', '') or article.description or article.title
        
        # Analyze content
        analysis = self.content_analyzer.analyze_content(content, metadata)
        
        # Check for duplicates using content hash
        is_duplicate = self.content_analyzer.detect_duplicates(analysis['content_hash'], existing_hashes)
    ...
    def _get_existing_content_hashes(self) -> List[str]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT content_hash FROM articles WHERE content_hash IS NOT NULL')
            return [row[0] for row in cursor.fetchall()]
    
    def get_high_quality_articles(self, min_quality: float = 0.7, limit: Optional[int] = None) -> List[Dict]:
    ...
    def get_articles_by_category(self, category: str, min_quality: float = 0.0) -> List[Dict]:
