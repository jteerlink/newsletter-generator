...
class MCPOrchestrator:
    ...
    async def execute_workflow(self, workflow: MCPWorkflow) -> Dict[str, Any]:
        ...
        try:
            ...
            for step_id in execution_order:
                ...
                try:
                    ...
                    if step.step_id in ["content_synthesis", "notion_publish"]:
                        ...
                        logger.warning(
                            f"Continuing workflow despite step failure: {
                                step.step_id}")
            ...
            logger.info(
                f"Workflow {
                    workflow.workflow_id} completed with status: {
                    workflow.status}")
            ...
            return {
                ...
                "execution_time": (
                    ...
                    workflow.created_at).total_seconds() if workflow.completed_at else None}
            ...
            return {
                ...
                "execution_time": (
                    workflow.completed_at -
                    workflow.created_at).total_seconds()}
    ...
    async def _execute_step(self, step: MCPWorkflowStep, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        @track_tool_call(
            ...
            # Pass step result or step ID as input
            input_data=step.result if step.result else step.step_id
        )
    ...
    async def _execute_github_step(self, step: MCPWorkflowStep, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "repositories": [
                {
                    # Use step_id for mock data
                    "name": f"mock-repo-{step.step_id}",
            ...
            "query": context.get(step.step_id, {}).get("query", "unknown")
        }

    def _calculate_execution_order(
            self, steps: List[MCPWorkflowStep]) -> List[str]:
    ...
    def _compile_workflow_results(
            self, workflow: MCPWorkflow) -> Dict[str, Any]:
        ...
        for step in workflow.steps:
            if step.result and isinstance(step.result, dict):
                if "results" in step.result:
                    ...
                    summary["total_results"] += len(
                        step.result["repositories"])

        # Get quality score
        quality_step = next(
            (s for s in workflow.steps if s.step_id == "quality_assessment"), None)
        if quality_step and quality_step.result:
            summary["quality_score"] = quality_step.result.get(
                "overall_score", 0.0)

        return summary

    def _create_publishing_summary(
            self, workflow: MCPWorkflow) -> Dict[str, Any]:
        ...
        for step in workflow.steps:
            if step.result and isinstance(step.result, dict):
                if "url" in step.result:
                    ...
                    summary["published_urls"].append(
                        f"notion://{step.result['page_id']}")

        # Check analytics
        analytics_step = next(
            (s for s in workflow.steps if s.step_id == "analytics_setup"), None)
        if analytics_step and analytics_step.result:
            summary["analytics_enabled"] = analytics_step.result.get(
                "tracking_enabled", False)

        return summary

    # Helper methods
    def _synthesize_multi_source_research(
            self, topic: str, source_results: List[Dict[str, Any]]) -> str:
        """Synthesize research from multiple sources."""
        ...
        if all_content:
            return f"Synthesized research on {
                topic}:\n\n" + "\n\n".join(all_content)
        else:
            return f"No content found for topic: {topic}"

    def _assess_content_quality(
            self, content: str, topic: str) -> Dict[str, Any]:
    ...
    def get_tool_usage_analytics(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            # Last 20 MCP calls
            "recent_mcp_usage": [entry.to_dict() for entry in mcp_entries[:20]]
        }
    ...
    def _analyze_workflow_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_type": max(
                workflow_types.keys(),
                key=lambda k: workflow_types[k]["count"]) if workflow_types else None}
    ...
    def _calculate_average_step_time(self, entries: List) -> float:
        ...
        total_time = sum(
            entry.execution_time for entry in entries if entry.execution_time is not None)
        valid_entries = len(
            [entry for entry in entries if entry.execution_time is not None])
