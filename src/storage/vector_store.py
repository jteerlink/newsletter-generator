...
class ChromaStorageProvider(StorageProvider):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Created/accessed collection: {self.config.collection_name}")
            except Exception as e:
            ...
            logger.info(f"Successfully initialized ChromaDB storage")
            return True
        except Exception as e:
    ...
    def add_document(self, content: str, metadata: DocumentMetadata) -> str:
        ...
        try:
            ...
            chunk_ids = [f"{metadata.doc_id}_chunk_{
        ...
        try:
            ...
            metadata = DocumentMetadata(
                ...
                content_type=DataType(
                    first_metadata["content_type"]),
                timestamp=datetime.fromisoformat(
                    first_metadata["timestamp"]),
                author=first_metadata.get("author"),
                tags=json.loads(
                    first_metadata["tags"]) if first_metadata["tags"] else [],
    ...
    def get_stats(self) -> StorageStats:
        ...
        try:
            ...
            total_cache_requests = len(
                self.embedding_cache) + len(self.query_cache)
            if total_cache_requests > 0:
                cache_hits = sum(
                    1 for v in self.embedding_cache.values() if v is not None)
        ...
        try:
            ...
            for i, (doc, metadata) in enumerate(
                ...
                doc_metadata = DocumentMetadata(
                    ...
                    tags=json.loads(
                        metadata["tags"]) if metadata["tags"] else [],
    ...
    def _get_embeddings_with_cache(self,
    ...
    def _build_where_clause(
            self, filters: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _process_search_results(
            self, results: Dict[str, Any], query: str, top_k: int) -> List[SearchResult]:
    ...
    def deduplicate(self, chunks: List[str]) -> List[int]:
        ...
        for cluster_id in range(n_clusters):
            cluster_indices = [i for i, label in enumerate(
                cluster_labels) if label == cluster_id]
            if cluster_indices:
