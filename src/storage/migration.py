...
from .base import StorageProvider, StorageConfig, DocumentMetadata, DataType
...
class MigrationManager:
    ...
    def create_migration_plan(self, source_provider: StorageProvider, 
                             target_provider: StorageProvider,
                             filters: Optional[Dict[str, Any]] = None) -> MigrationPlan:
    ...
    def execute_migration(self, plan: MigrationPlan, 
        ...
        try:
            ...
            for i in range(0, len(all_documents), batch_size):
                ...
                for doc_content, doc_metadata in batch:
                    try:
                        ...
                        plan.target_provider.add_document(doc_content, doc_metadata)
                        plan.migrated_documents += 1
                        
                        if progress_callback:
                            progress = (plan.migrated_documents / plan.total_documents) * 100
                            progress_callback(progress, plan.migrated_documents, plan.total_documents)
                        
                    except Exception as e:
                        plan.failed_documents += 1
                        error_msg = f"Failed to migrate document {doc_metadata.doc_id}: {e}"
                        plan.errors.append(error_msg)
                        logger.error(error_msg)
                
                # Log progress
                logger.info(f"Migrated {plan.migrated_documents}/{plan.total_documents} documents")
            ...
            logger.info(f"Migration {plan.migration_id} completed: "
                       f"{plan.migrated_documents} successful, {plan.failed_documents} failed")
    ...
    def _get_all_documents(self, provider: StorageProvider) -> List[tuple]:
        ...
        try:
            ...
            if hasattr(provider, 'migrate'):
                ...
                logger.warning("Provider does not support document enumeration")
        ...
        try:
            ...
            if plan.migrated_documents == plan.total_documents:
                ...
                validation_result["errors"].append(
                    f"Document count mismatch: migrated {plan.migrated_documents}, "
    ...
    def rollback_migration(self, plan: MigrationPlan) -> bool:
        ...
        try:
            ...
            # Clear target provider (this would need to be implemented by each provider)
            if hasattr(plan.target_provider, 'clear_all'):
                plan.target_provider.clear_all()
            else:
                logger.warning("Target provider does not support clear_all operation")
            
            # Remove from history
            self.migration_history = [m for m in self.migration_history if m.migration_id != plan.migration_id]
            self._save_migration_history()
            
            logger.info(f"Rollback completed for migration {plan.migration_id}")
            return True
            
        except Exception as e:
            logger.error(f"Rollback failed for migration {plan.migration_id}: {e}")
            return False
    
    def get_migration_history(self) -> List[MigrationPlan]:
        """Get migration history."""
        return sorted(self.migration_history, key=lambda m: m.start_time or datetime.min, reverse=True)
    
    def get_migration_stats(self) -> Dict[str, Any]:
        """Get overall migration statistics."""
        total_migrations = len(self.migration_history)
        successful_migrations = sum(1 for m in self.migration_history if m.failed_documents == 0)
        total_documents_migrated = sum(m.migrated_documents for m in self.migration_history)
        total_documents_failed = sum(m.failed_documents for m in self.migration_history)
...
class DataFormatConverter:
    ...
    def convert_metadata_format(old_metadata: Dict[str, Any], 
        ...
        doc_id = old_metadata.get("doc_id") or old_metadata.get("id") or str(uuid.uuid4())
        title = old_metadata.get("title") or old_metadata.get("name") or "Untitled"
        source = old_metadata.get("source") or old_metadata.get("url") or "unknown"
        
        # Parse timestamp
        timestamp_str = old_metadata.get("timestamp") or old_metadata.get("date")
        if timestamp_str:
            try:
        ...
        # Parse content type
        content_type_str = old_metadata.get("content_type") or old_metadata.get("type") or "text"
        try:
