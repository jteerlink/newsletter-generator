...
class StorageManager:
    ...
    def _merge_search_results(self, results: List[SearchResult], top_k: int) -> List[SearchResult]:
        """Merge and deduplicate search results."""
        # Group by doc_id and take the best score
        doc_results = {}
        for result in results:
            if result.doc_id not in doc_results or result.similarity_score > doc_results[result.doc_id].similarity_score:
    ...
    def get_combined_stats(self) -> StorageStats:
        ...
        for provider in self.providers:
            try:
                ...
                combined_stats.cache_hit_rate = max(combined_stats.cache_hit_rate, 
                                                  stats.cache_hit_rate)
                combined_stats.average_query_time_ms = max(combined_stats.average_query_time_ms,
