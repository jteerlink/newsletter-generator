...
from .tool_usage_tracker import get_tool_tracker, ToolUsageTracker
...
class ToolUsageAnalyticsDashboard:
    ...
    def __init__(self, 
                 ...
                 feedback_system: Optional[FeedbackLearningSystem] = None):
    ...
    def refresh_dashboard_data(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        # Get core analytics
        system_analytics = self.tool_tracker.generate_usage_analytics(hours_back)
    ...
    def get_tool_performance_report(self, tool_name: str, hours_back: int = 168) -> Dict[str, Any]:
        """Generate detailed performance report for a specific tool."""
        tool_entries = [
            entry for entry in self.tool_tracker.get_tool_usage_history(hours_back=hours_back)
        ...
        # Performance metrics
        execution_times = [entry.execution_time for entry in tool_entries if entry.execution_time]
        success_count = len([entry for entry in tool_entries if entry.status == "success"])
        ...
        report = {
            ...
            "usage_statistics": {
                ...
                "success_rate": success_count / len(tool_entries) if tool_entries else 0,
            ...
            "usage_patterns": {
                ...
                "peak_usage_hour": max(usage_by_hour.items(), key=lambda x: x[1])[0] if usage_by_hour else None
            },
            "error_analysis": {
                "total_errors": len(tool_entries) - success_count,
                "error_rate": (len(tool_entries) - success_count) / len(tool_entries) if tool_entries else 0,
                "error_types": dict(error_types)
            },
            "recommendations": self._generate_tool_recommendations(tool_name, tool_entries)
        }
        
        return report
    
    def get_agent_performance_comparison(self, hours_back: int = 168) -> Dict[str, Any]:
        """Compare performance across different agents."""
        all_entries = self.tool_tracker.get_tool_usage_history(hours_back=hours_back)
        ...
        for agent_name, metrics in agent_metrics.items():
            comparison[agent_name] = {
                ...
                "average_execution_time": metrics["total_execution_time"] / metrics["successful_calls"] if metrics["successful_calls"] > 0 else 0,
                "unique_tools_used": len(metrics["tools_used"]),
                "active_sessions": len(metrics["sessions"]),
        ...
        return {
            ...
            "rankings": {
                "by_efficiency": [{"agent": name, "score": data["efficiency_score"]} for name, data in ranked_agents],
                "by_success_rate": sorted(comparison.items(), key=lambda x: x[1]["success_rate"], reverse=True),
                "by_tool_diversity": sorted(comparison.items(), key=lambda x: x[1]["unique_tools_used"], reverse=True)
            },
            "insights": self._generate_agent_insights(comparison)
        }
    
    def generate_executive_summary(self, hours_back: int = 168) -> Dict[str, Any]:
        ...
        # Quality insights
        quality_insights = {}
        if self.feedback_system and self.dashboard_data.get("quality_correlations"):
            quality_insights = self.dashboard_data["quality_correlations"].get("analysis_summary", {})
    ...
    def export_analytics_data(self, 
        ...
        if format_type.lower() == "json":
            ...
            tool_entries = self.tool_tracker.get_tool_usage_history(hours_back=hours_back)
    ...
    def _get_agent_analytics(self, hours_back: int) -> Dict[str, Any]:
        """Get agent-specific analytics."""
        entries = self.tool_tracker.get_tool_usage_history(hours_back=hours_back)
        
        agent_breakdown = defaultdict(lambda: {"calls": 0, "success": 0, "avg_time": 0, "tools": set()})
        ...
        for entry in entries:
            ...
            if entry.execution_time:
                ...
                agent_breakdown[entry.agent_name]["avg_time"] = (current_avg * (current_count - 1) + entry.execution_time) / current_count
        ...
        for agent, data in agent_breakdown.items():
            result[agent] = {
                ...
                "success_rate": data["success"] / data["calls"] if data["calls"] > 0 else 0,
    ...
    def _get_performance_insights(self, hours_back: int) -> Dict[str, Any]:
        ...
        execution_times = [entry.execution_time for entry in entries if entry.execution_time]
        success_count = len([entry for entry in entries if entry.status == "success"])
        ...
        return {
            ...
            "overall_success_rate": success_count / len(entries),
            "average_execution_time": sum(execution_times) / len(execution_times) if execution_times else 0,
    ...
    def _get_workflow_analytics(self, hours_back: int) -> Dict[str, Any]:
        ...
        workflow_data = defaultdict(lambda: {"calls": 0, "tools": set(), "duration": 0})
    ...
    def _generate_dashboard_summary(self, system_analytics: Dict, agent_analytics: Dict, performance_insights: Dict) -> Dict[str, Any]:
    ...
    def _get_most_used_tool(self, entries: List) -> str:
        ...
        return max(tool_counts.items(), key=lambda x: x[1])[0] if tool_counts else "None"
    
    def _get_system_status(self) -> str:
        """Determine current system status."""
        recent_entries = self.tool_tracker.get_tool_usage_history(hours_back=0.5)  # Last 30 minutes
    ...
    def _calculate_efficiency_score(self, metrics: Dict) -> float:
        ...
        error_penalty = metrics["errors"] / metrics["total_calls"] if metrics["total_calls"] > 0 else 0
        
        # Weighted efficiency score
        score = (success_rate * 0.5) + (min(tool_diversity / 5, 1.0) * 0.3) - (error_penalty * 0.2)
        return max(0, min(1, score))  # Clamp between 0 and 1
    
    def _generate_tool_recommendations(self, tool_name: str, entries: List) -> List[str]:
        ...
        if success_rate < 0.8:
            recommendations.append(f"Investigate reliability issues - success rate is {success_rate:.1%}")
        
        if avg_time > 10.0:
            recommendations.append(f"Optimize performance - average execution time is {avg_time:.1f}s")
        
        return recommendations if recommendations else ["Tool performance is within acceptable parameters"]
    ...
    def _generate_agent_insights(self, comparison: Dict) -> List[str]:
        ...
        best_agent = max(comparison.items(), key=lambda x: x[1]["efficiency_score"])
        worst_agent = min(comparison.items(), key=lambda x: x[1]["efficiency_score"])
        
        insights.append(f"Top performing agent: {best_agent[0]} (efficiency: {best_agent[1]['efficiency_score']:.1%})")
        
        if best_agent[1]["efficiency_score"] - worst_agent[1]["efficiency_score"] > 0.2:
            insights.append(f"Significant performance gap detected between {best_agent[0]} and {worst_agent[0]}")
    ...
    def _analyze_performance_trend(self, hours_back: int) -> str:
        ...
        recent_entries = self.tool_tracker.get_tool_usage_history(hours_back=half_period)
        older_entries = self.tool_tracker.get_tool_usage_history(hours_back=hours_back)
    ...
    def _get_top_performing_insight(self) -> str:
        ...
        if most_reliable:
            return f"Most reliable tool: {most_reliable[0].get('tool', 'Unknown')} with {most_reliable[0].get('success_rate', 0):.1%} success rate"
    ...
    def _get_improvement_opportunity_insight(self) -> str:
        ...
        performance_insights = self.dashboard_data.get("performance_insights", {})
        bottlenecks = performance_insights.get("performance_bottlenecks", {})
        
        if bottlenecks:
            slowest_tool = max(bottlenecks.items(), key=lambda x: x[1]["average_slow_time"])
            return f"Optimization opportunity: {slowest_tool[0]} averaging {slowest_tool[1]['average_slow_time']:.1f}s"
    ...
    def _generate_executive_recommendations(self) -> List[str]:
        ...
        if success_rate < 0.9:
            recommendations.append("Investigate and address system reliability issues")
        ...
        quality_correlations = self.dashboard_data.get("quality_correlations", {})
        if quality_correlations and quality_correlations.get("analysis_summary", {}).get("recommendations"):
            recommendations.extend(quality_correlations["analysis_summary"]["recommendations"][:2])
        
        return recommendations if recommendations else ["System operating within optimal parameters"]
    
    def _determine_system_health(self, system_analytics: Dict, performance_insights: Dict) -> str:
        """Determine overall system health status."""
        success_rate = system_analytics.get("overall_success_rate", 0)
        avg_time = system_analytics.get("average_execution_time", 0)
        bottlenecks = len(performance_insights.get("performance_bottlenecks", {}))
    ...
    def _generate_alerts(self, system_analytics: Dict, performance_insights: Dict) -> List[str]:
        """Generate system alerts."""
        alerts = []
        
        success_rate = system_analytics.get("overall_success_rate", 0)
        if success_rate < 0.8:
            alerts.append(f"LOW SUCCESS RATE: {success_rate:.1%} - investigate immediately")
        
        avg_time = system_analytics.get("average_execution_time", 0)
        if avg_time > 10.0:
            alerts.append(f"HIGH RESPONSE TIME: {avg_time:.1f}s - performance issue detected")
        
        bottlenecks = performance_insights.get("performance_bottlenecks", {})
        if len(bottlenecks) > 5:
            alerts.append(f"MULTIPLE BOTTLENECKS: {len(bottlenecks)} tools showing performance issues")
    ...
    def _identify_top_performers(self, system_analytics: Dict) -> List[str]:
        ...
        performers = []
        if most_reliable:
            performers.append(f"Most Reliable: {most_reliable[0].get('tool', 'Unknown')}")
        if fastest:
            performers.append(f"Fastest: {fastest[0].get('tool', 'Unknown')}")
        
        return performers
    
    def _identify_improvement_areas(self, performance_insights: Dict) -> List[str]:
        """Identify areas for improvement."""
        areas = []
        
        bottlenecks = performance_insights.get("performance_bottlenecks", {})
        if bottlenecks:
            areas.append(f"Performance: {len(bottlenecks)} tools need optimization")
    ...
    def _identify_reliable_tools(self, entries: List) -> List[Dict[str, Any]]:
        ...
        return sorted(reliable_tools, key=lambda x: x["success_rate"], reverse=True)[:5]

def create_analytics_dashboard(tool_tracker: Optional[ToolUsageTracker] = None,
                             feedback_system: Optional[FeedbackLearningSystem] = None) -> ToolUsageAnalyticsDashboard:
