...
class ToolUsageLogger:
    ...
    def __init__(self,
        ...
        logger.info(
            f"ToolUsageLogger initialized - tracking to {self.log_file}")
    ...
    def _load_recent_entries(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._recent_entries)
    ...
    def track_tool_usage(self,
        ...
        try:
            logger.debug(f"Starting tool usage tracking: {
                         tool_name} by {agent_name}")
            yield
    ...
    def log_tool_usage(self, entry: ToolUsageEntry):
        ...
        with self._lock:
            ...
            logger.debug(f"Logged tool usage: {entry.tool_name} by {
                         entry.agent_name} - {entry.status.value}")
    ...
    def _update_performance_cache(self, entry: ToolUsageEntry):
        ...
        metrics.min_execution_time = min(
            metrics.min_execution_time, entry.execution_time)
        metrics.max_execution_time = max(
            metrics.max_execution_time, entry.execution_time)
        ...
        if entry.error_message:
            ...
            metrics.error_patterns[error_key] = metrics.error_patterns.get(
                error_key, 0) + 1

        # Track usage trends (hourly)
        hour_key = entry.timestamp.strftime("%Y-%m-%d %H:00")
        metrics.usage_trends[hour_key] = metrics.usage_trends.get(
            hour_key, 0) + 1
    ...
    def get_tool_performance(
            self,
            tool_name: str) -> Optional[ToolPerformanceMetrics]:
    ...
    def generate_usage_analytics(
            self, hours_back: int | None = None) -> ToolUsageAnalytics:
        ...
        for tool in tool_usage_counts:
            ...
            tool_success_rates[tool] = success_count / \
        ...
        least_reliable_tools = sorted(
            tool_success_rates.items(),
            key=lambda x: x[1])[
            :5]
        least_reliable_tools = [tool for tool,
                                rate in least_reliable_tools if rate < 0.9]
        ...
        # Time period summary
        time_period_summary = self._generate_time_period_summary(
            recent_entries)
        ...
        return ToolUsageAnalytics(
            ...
            average_success_rate=sum(
                tool_success_rates.values()) /
            len(tool_success_rates) if tool_success_rates else 0.0,
    ...
    def _generate_time_period_summary(
            self, entries: List[ToolUsageEntry]) -> Dict[str, Dict[str, Any]]:
        ...
        for hour, data in hourly_data.items():
            summary[hour] = {
                ...
                data["count"] if data["count"] > 0 else 0.0,
    ...
    def save_metrics_report(self, output_file: str | None = None):
        ...
        if output_file is None:
            output_file = f"logs/tool_usage_report_{
                datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

        analytics = self.generate_usage_analytics()
        tool_metrics = {
            name: asdict(metrics) for name,
            metrics in self._performance_cache.items()}
